...
function MCMCiFFBS_(N, 
    ...
    for iter in 1:N
        ...
        if (iter+1) % 100 == 0
            println("Saving checkpoint at iteration $(iter+1)...")
            
            # Save as JLD2 (native Julia, faster)
            jldsave(joinpath(path, "checkpoint.jld2");
                    out = out[1:iter+1, :],
                    logPostPerIter = logPostPerIter[1:iter+1],
                    logLikPerIter = logLikPerIter[1:iter+1],
                    nSus = nSus[:, 1:iter+1],
                    nExp = nExp[:, 1:iter+1],
                    nInf = nInf[:, 1:iter+1],
                    iter = iter+1)
        ...
        # Save every blockSize iterations (permanent blocks, like C++)
        if (iter+1) % blockSize == 0
            ...
            # Save block-specific outputs - JLD2 format
            jldsave(joinpath(block_dir, "block_results.jld2");
                    nSusTested = nSusTested,
                    nExpTested = nExpTested,
                    nInfTested = nInfTested,
                    nSusTestedPerGroup = nSusTestedPerGroup,
                    nExpTestedPerGroup = nExpTestedPerGroup,
                    nInfTestedPerGroup = nInfTestedPerGroup,
                    nSusByGroup = nSusByGroup,
                    nExpByGroup = nExpByGroup,
                    nInfByGroup = nInfByGroup,
                    infTimes = infTimes,
                    infectivityTimes = infectivityTimes,
                    deathTimes = deathTimes,
                    AcontribPop = AcontribPop,
                    AcontribPopTime = AcontribPopTime,
                    AcontribGroup = AcontribGroup,
                    AcontribGroupTime = AcontribGroupTime)
        end
